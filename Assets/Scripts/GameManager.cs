    ...
    public void Loading()
    {
        if (File.Exists(Application.persistentDataPath + "/SaveFile.octo"))
        {
            ...
            for (int i = 0; i < data.savedTerritories.Count; i++)
            {
                ...
                {
                    if (data.savedTerritories[i].name == TerritoryManager.instance.territoryList[j].GetComponent<TerritoryHandler>().territory.name)
                    {
                        TerritoryManager.instance.territoryList[j].GetComponent<TerritoryHandler>().territory = data.savedTerritories[i];
                    }
                }
            }
